...
use lob_replay::complete_replay::{CompleteRunReplaySelectionV1, replay_complete_run};
use std::env;
...
fn run() -> Result<()> {
    ...
    let selection_path = args.next().map(PathBuf::from).ok_or_else(|| {
        format!(
            "usage: {} <complete-run-replay-selection.json> [new-report.json]",
            executable.display()
        )
    })?;
    ...
    if let Some(path) = output.as_ref() {
        ...
        if resolved_target(path)?.starts_with(&run) {
            return Err("replay report must remain outside immutable source run".to_owned());
        }
    }
    let report = replay_complete_run(&selection)?;
    let mut bytes = serde_json::to_vec_pretty(&report)
        .map_err(|error| format!("serialize complete replay report: {error}"))?;
    bytes.push(b'\n');
    ...
    std::io::stdout()
        .write_all(&bytes)
        .map_err(|error| format!("write complete replay report: {error}"))
}

fn main() {
    if let Err(error) = run() {
        eprintln!("complete-replay: {error}");
        std::process::exit(2);
    }
}
